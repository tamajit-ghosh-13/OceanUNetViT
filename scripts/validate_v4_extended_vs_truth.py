...
def validate_v4_extended_era(tag, era_name):
    ...
    for t in range(len(dates)):
        daily_interp[t] = RegularGridInterpolator((depths, lat_grid, lon_grid), preds_v4_c[t], method="linear", bounds_error=False, fill_value=None)
    ...
    header_pred = "v4_extended 🚀 (°C)"
    header_err = "Error (RMSE)"
    header_corr = "Corr (r)"

    print("=" * 115)
    print(f"🚀 OCEANUNETVIT v4_EXTENDED VS PHYSICAL ARGO GROUND TRUTH: {era_name.upper()}")
    print("=" * 115)
...
if __name__ == "__main__":
    ...
    for tag, name in eras:
        validate_v4_extended_era(tag, name)
