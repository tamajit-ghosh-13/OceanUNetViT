...
def validate_tribreed_era(tag, era_name):
    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")

    # Load 3 Constituent Models
    ...
    for b in range(0, T, 4):
        ...
        with torch.no_grad():
            ...
            p4 = m_v4(torch.from_numpy(proc.astype(np.float32)).to(device)).cpu().numpy()
    ...
    # 3. Tri-Breed Depth-Wise Optimal Convex Combination
    depths = np.array(STANDARD_DEPTH_LEVELS_M, dtype=float)
    preds_tri_c = np.zeros_like(preds_ft_c)
    for d_idx, d_val in enumerate(depths):
        w = TRI_WEIGHTS[int(d_val)]
        preds_tri_c[:, d_idx] = w[0] * preds_ft_c[:, d_idx] + w[1] * preds_v3_c[:, d_idx] + w[2] * preds_v4_c[:, d_idx]
    ...
    for t in range(len(dates)):
        daily_interp[t] = RegularGridInterpolator((depths, lat_grid, lon_grid), preds_tri_c[t], method="linear", bounds_error=False, fill_value=None)
    ...
    header_pred = "Tri-Breed AI 🧬 (°C)"
    header_err = "Error (RMSE)"
    header_corr = "Corr (r)"

    print("=" * 115)
    print(f"🧬 TRI-BREED ENSEMBLE MODEL VS PHYSICAL ARGO GROUND TRUTH: {era_name.upper()}")
    print("=" * 115)
    print(f"{header_d:>10} | {header_obs:>12} | {header_truth:>18} | {header_pred:>22} | {header_err:>15} | {header_corr:>10}")
    print("-" * 115)
    all_rmse, all_corr = [], []
    for d in bin_labels:
        ...
        str_r = f"{rmse:.4f}°C"
        str_c = f"{corr:.3f}"
        print(f"{d:>10d} | {len(t):>12,d} | {str_t:>18} | {str_p:>22} | {str_r:>15} | {str_c:>10}")
    print("-" * 115)
    mean_rmse_str = f"{np.mean(all_rmse):.4f}°C"
    mean_corr_str = f"{np.mean(all_corr):.3f}"
    print(f"{chr(39)}OVERALL{chr(39):>3} | {len(df):>12,d} | {chr(45):>18} | {chr(45):>22} | {mean_rmse_str:>15} | {mean_corr_str:>10}")
    print("=" * 115 + "\n")

if __name__ == "__main__":
    validate_tribreed_era("nov16", "November 2016 (Historic Negative IOD Era)")
    validate_tribreed_era("jun20", "June 2020 (Super Cyclone Amphan / SW Monsoon Era)")
